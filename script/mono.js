...
const img = ([inverse, contrast, dark, light, i], _data) => {
    ...
    mono(o, dark, light, contrast, inverse);
...
export const load = (e, next, back, d) => {
    ...
    scroll('mono', data.imgs.length, e);
    let canvas = e.querySelector('canvas');
    let { _w, _h } = data;
    canvas.height = _h;
    canvas.width = _w;

    changed('mono', () => draw(canvas, img(shot('mono'))));
...
export const next = done => {
    ...
    let [inverse] = shot('mono');
    data.inverse = !!inverse;
    return done(data);
}

export const process = (data, mono) => {
    let out = { ...data };
    out.imgs = [];

    let d = shot('mono', mono).slice(0, -1);
